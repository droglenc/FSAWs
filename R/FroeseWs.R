...
#' 
#' @description Computes the standard weight equation using the geometric mean of a and the mean of b from weight-length regression equations as described in Froese (2006).
#' 
#' @details The main function computes the mean of the \eqn{log_{10}(a)} and b values for the standard weight equation as detailed in Froese (2006).  Note that log(a) and b must be from the regression of \eqn{log_{10}(W)} on \eqn{log_{10}(L)} where W is measured in grams and L is the total length measured in mm.
#' 
...
#' @param x An object saved from the \code{FrowseWs()} call (i.e., of class \code{Froese}).
...
#' @param object An object saved from \code{FroeseWs()} (i.e., of class \code{FroeseWs}).
